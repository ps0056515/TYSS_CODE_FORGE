...
export default async function AtRiskStudentsPage() {
  ...
  const list = await getAtRiskStudentsList(assignments, subs);
    ...
    <Container className="py-8 md:py-10">
      ...
      <header className="mb-8">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-medium uppercase tracking-widest text-muted">At-risk</p>
            <h1 className="mt-1 text-2xl font-bold tracking-tight text-text">At-risk students</h1>
            <p className="mt-1 text-sm text-muted">
              Flagged by rules: no activity 3d, due soon (48h), or late
            </p>
          </div>
        </div>
      </header>
      ...
      {list.length === 0 ? (
        <DashboardCard className="p-8 text-center">
          <p className="text-muted">No at-risk students flagged.</p>
        </DashboardCard>
      ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-black/5 dark:bg-white/5">
                  ...
                  <th className="text-left p-4 font-medium text-text">Reasons</th>
